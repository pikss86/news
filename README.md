...
В активном OpenSpec остаётся один формальный пункт ручной приёмки VPN-доступа и полного Telegram
code/2FA-сценария.
Система получает Telegram updates, сохраняет raw events, строит текущую проекцию и неизменяемую
историю сообщений, учитывает media и позволяет просматривать собранные данные через VPN.
...
из admin-сессии внутри VPN, а путь дополнительно проверяется на принадлежность каталогу TDLib.
...
диске файл. Раздел доступен только внутри VPN после входа администратора, не позволяет удалять или
...
`FileHandlerRegistry`; маршруты VPN, аутентификация и проверка пути при этом переиспользуются.
