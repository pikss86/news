...
| --- | --- |
| [README.md](README.md) | Как устроена и эксплуатируется текущая реализация? |
| [CONCEPT.md](CONCEPT.md) | Зачем существует система и какие принципы нельзя нарушать? |
| [ROADMAP.md](ROADMAP.md) | Что уже готово, что делать следующим и по каким критериям считать этап завершённым? |
| [openspec/specs/](openspec/specs/) | Какое поведение уже принято как действующий технический контракт? |
| [openspec/changes/](openspec/changes/) | Какие конкретные изменения сейчас проектируются или проходят приёмку? |

При расхождении описание текущего поведения подтверждается кодом, тестами и основными
спецификациями OpenSpec. Roadmap задаёт направление и порядок работ, но не подтверждает, что
функция уже реализована. Правила работы с OpenSpec и статус активных изменений собраны в
[openspec/README.md](openspec/README.md).

## Текущий статус

Milestone 0.1 — работающий collector и защищённый интерфейс оператора — реализован и запущен.
В активном OpenSpec остаётся один формальный пункт ручной приёмки полного Telegram
code/2FA-сценария с настоящими credentials.
Система получает Telegram updates, сохраняет raw events, строит текущую проекцию и неизменяемую
историю сообщений, учитывает media и позволяет просматривать собранные данные в read-only
интерфейсе.
Аналитический слой — сюжеты, утверждения, проверка свидетельств и alerts — ещё не реализован.

Следующая цель — milestone 0.2: измеримая свежесть и наблюдаемое покрытие, воспроизводимое
восстановление и повторная обработка данных, контролируемый historical backfill и
эксплуатационные сигналы о проблемах сбора. Подробности и критерии готовности находятся в
...
Удаление сообщения только из локального кэша TDLib (`from_cache=true`) сохраняется как raw event,
но не считается удалением публикации и не меняет нормализованную проекцию.
...
Нужны Linux-host, Docker Engine и Docker Compose.
...
Откройте административную страницу развёрнутого сервиса. Дальнейший сценарий полностью
браузерный:
...
требует admin-сессию и не содержит операций изменения или удаления данных.
Если защищённую ссылку открыть без активной сессии, после ввода пароля интерфейс возвращает на
исходную страницу. Карточка сообщения показывает прямую ссылку на оригинал в Telegram, когда её
можно однозначно построить. Для приватного канала она откроется только у участника этого канала.
...
После завершения появляется кнопка «Открыть»: безопасные изображения, аудио, видео, PDF и простой
текст открываются в новой вкладке, остальные форматы предлагаются как файл. Выдача доступна только
из admin-сессии, а путь дополнительно проверяется на принадлежность каталогу TDLib.

Отдельный раздел «Кэш TDLib» показывает фактическое содержимое persistent-каталога `files` без
привязки к индексу PostgreSQL. В нём можно переходить по папкам и открывать любой находящийся на
диске файл. Раздел доступен только после входа администратора, не позволяет удалять или
изменять файлы и отклоняет абсолютные пути, переходы наружу и symlink за пределы TDLib cache.

Файловый браузер использует реестр потоковых обработчиков типов. Встроенный ZIP-обработчик
распознаёт архивы по расширению или сигнатуре, поэтому работает и с безымянными файлами TDLib.
Архив открывается как виртуальная иерархия; выбранный `path/to/document.pdf` распаковывается по
частям непосредственно в HTTP-ответ с типом `application/pdf`. Временная распакованная копия не
создаётся, исходный ZIP остаётся в persistent cache. Записи с `..`, абсолютными путями,
шифрованием, чрезмерным распакованным размером или подозрительным коэффициентом сжатия блокируются.
Новые обработчики добавляются реализацией контракта `StreamingFileHandler` и регистрацией в
`FileHandlerRegistry`; аутентификация и проверка пути при этом переиспользуются.
Страница `/browser/cache/handlers` показывает все активные обработчики, правила распознавания,
защитные ограничения и рекурсивный список подходящих файлов во всём кэше. Поэтому архив можно
открыть напрямую, даже если он лежит во вложенной папке или не имеет расширения. Распознанные
строки дополнительно выделены фоном, значком `📦`, цветным бейджем и основной кнопкой.

Скачивание media включается флагом на странице. Приоритет 1–32 задаёт порядок запросов; фильтров
размера и типа пока нет, поэтому следите за свободным местом. Автоматическая очистка файлов в
TDLib отключена: скачанный кэш сохраняется в persistent volume и интерфейс не содержит операции
удаления. Обычные restart, recreate и `docker compose down` кэш не удаляют. Ручное удаление volume
или команда `docker compose down -v` по-прежнему уничтожат его вместе с остальными данными volume.

Внизу страницы каждого чата есть кнопка «Загрузить ещё 100 из Telegram». Первый запрос начинается
с самого старого сообщения этого чата, уже сохранённого в PostgreSQL, а следующие нажатия сдвигают
границу дальше в прошлое. Ответ `getChatHistory` целиком сохраняется как raw event, затем входящие в
него сообщения проходят обычную нормализацию и инвентаризацию файлов. Повторно попавшие в порцию
сообщения не создают лишних нормализованных версий. TDLib может вернуть меньше 100 записей даже до
конца истории; кнопку можно нажать снова. На странице чата показываются последние 100 сообщений,
а вся уже собранная история доступна в постраничном разделе «Поиск по сообщениям».
...
Панель использует стойкий hash пароля, server-side sessions, one-time CSRF, rate limit и cookies
без доступа из client-side scripts. Секреты шифруются на диске и маскируются в
HTML/status/errors. Root или
...
OpenSpec change описывает одно ограниченное изменение; общий порядок продуктовых этапов задаёт
[ROADMAP.md](ROADMAP.md), а актуальный реестр OpenSpec-артефактов —
[openspec/README.md](openspec/README.md).
...
- Ручная загрузка истории получает наблюдаемое текущее представление старых сообщений. Она не
  восстанавливает прежние редакции и удаления, которые Telegram больше не отдаёт аккаунту.
